...
public:
    const int INF = 2e9;
    long long runDijkstra(const vector<vector<pair<int, int>>>& graph, int n, int source, int destination) {
    ...
    vector<vector<int>> modifiedGraphEdges(int n, vector<vector<int>>& edges, int source, int destination, int target) {
        ...
        // initial distance without -1 weights
        int currentShortestDistance = runDijkstra(graph, n, source, destination);
        ...
        for(auto& edge: edges) {
            ...
            if(!matchesTarget) {
                int newDistance = runDijkstra(graph, n, source, destination);
